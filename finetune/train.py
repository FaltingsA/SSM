...
def main(config: DictConfig):
    ...
    if config.model.parseq_pretrained:
        parseq_checkpoint = torch.load(config.model.parseq_checkpoint,map_location='cpu')
        parseq_dict = parseq_checkpoint['state_dict']
        model_dict = model.state_dict()
        parseq_dict = {k: v for k, v in parseq_dict.items() if k in model_dict }
        print("read a complete pretrained model,update layer nums:",len(parseq_dict.keys()))
        model_dict.update(parseq_dict)
    ...
    if config.model.encoder_pretrained:
        ...
        if 'state_dict' in encoder_checkpoint.keys() and config.model.encoder_checkpoint.endswith('union14m.pth'):
        ...
        elif 'sdr' in config.model.encoder_checkpoint:
            pretrained_dict = encoder_checkpoint['model']
            model_dict = model.state_dict()
            pretrained_dict = {k: v for k, v in pretrained_dict.items() \
                                if  k in model_dict }#filter out unnecessary keys
            print("readi pretrain encoder from SDR, update layer nums:",len(pretrained_dict.keys())) #and k!='pos_embed'
            ...
            pretrained_dict = {('encoder.'+k): v for k, v in pretrained_dict.items() \
                                if (('encoder.'+k) in model_dict ) and k!='pos_embed'}#filter out unnecessary keys
