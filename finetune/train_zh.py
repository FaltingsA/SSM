...
from strhub.data.module import ChineseSceneTextDataModule
...
def main(config: DictConfig):
    ...
    if config.model.parseq_pretrained:
        print("read a complete pretrained parseq model")
    ...
    if config.model.encoder_pretrained:
        ...
        if 'state_dict' in encoder_checkpoint.keys() and config.model.encoder_checkpoint.endswith('union14m.pth'):
            ...
            pretrained_dict = {('encoder.'+k): v for k, v in pretrained_dict.items() \
                                if (('encoder.'+k) in model_dict )}#filter out unnecessary keys
    ...
    datamodule: ChineseSceneTextDataModule = hydra.utils.instantiate(config.data)
