...
def main(config: DictConfig):
    ...
    if config.model.ckpt_pretrained:
        print("read a complete pretrained ckpt model")
        ckpt_checkpoint = torch.load(config.model.ckpt_checkpoint,map_location='cpu')
        ckpt_dict = ckpt_checkpoint['state_dict']
        model_dict = model.state_dict()
        ckpt_dict = {k: v for k, v in ckpt_dict.items() if k in model_dict }
        model_dict.update(ckpt_dict)
        model.load_state_dict(model_dict)

    if config.model.encoder_pretrained:
        encoder_checkpoint = torch.load(config.model.encoder_checkpoint,map_location='cpu')
        
        if 'state_dict' in encoder_checkpoint.keys() and config.model.encoder_checkpoint.endswith('union14m.pth'):
            pretrained_dict = encoder_checkpoint['state_dict']
            model_dict = model.state_dict()

            pretrained_dict = {'encoder' + k[8:]: v for k, v in pretrained_dict.items() \
                                if k.startswith('backbone') and (('encoder' + k[8:]) in model_dict) and (k[8:] != '.pos_embed') }#filter out unnecessary keys
            
            print("readi pretrain encoder from MMOCRMAE, update layer nums:",len(pretrained_dict.keys())) #and k!='pos_embed'

            model_dict.update(pretrained_dict)

            model.load_state_dict(model_dict)

        elif 'dig' in config.model.encoder_checkpoint:
            pretrained_dict = encoder_checkpoint['model']
            model_dict = model.state_dict()
            pretrained_dict = {k: v for k, v in pretrained_dict.items() \
                                if  k in model_dict }#filter out unnecessary keys
            print("readi pretrain encoder from DiG, update layer nums:",len(pretrained_dict.keys())) #and k!='pos_embed'

            model_dict.update(pretrained_dict)

            model.load_state_dict(model_dict, strict=False)

        elif 'trb' in config.model.encoder_checkpoint:
            pretrained_dict = encoder_checkpoint['model']
            model_dict = model.state_dict()

            pretrained_dict = {('model.FeatureExtraction' + k[7:]): v for k, v in pretrained_dict.items() \
                                if k.startswith('encoder') and (('model.FeatureExtraction' + k[7:]) in model_dict)}#filter out unnecessary keys
            print("readi pretrain encoder from MDR, update layer nums:",len(pretrained_dict.keys())) #and k!='pos_embed'
            ...
            pretrained_dict = {('model' + k[7:]): v for k, v in pretrained_dict.items() \
                                if k.startswith('encoder') and (('model' + k[7:]) in model_dict)}#filter out unnecessary keys
            print("readi pretrain encoder from MDR, update layer nums:",len(pretrained_dict.keys())) #and k!='pos_embed'
