...
def save_model_best(args, epoch, model, model_without_ddp, optimizer, loss_scaler, best_metric):
    best_dir = Path(args.best_dir)
    epoch_name = str(epoch)
    if args.pixel_type == 'SR':
        best_psnr = best_metric['CUR_PSNR']
        best_ssim = best_metric['CUR_SSIM']
        checkpoint_paths = [best_dir / ('psnr-%.4f-ssim-%.4f-checkpoint-%s.pth' %(best_psnr,best_ssim,epoch_name))]
    elif args.pixel_type == 'Seg':
        best_IoU = best_metric['BEST_IoU']
        checkpoint_paths = [best_dir / ('IoU-%.4f-checkpoint-%s.pth' %(best_IoU,epoch_name))]
