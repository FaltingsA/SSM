...
class CVRandomRotation(object):
    ...
    def __call__(self, img, mask):
        ...
        return cv2.warpAffine(img, M, (dst_w, dst_h), flags=flags, borderMode=cv2.BORDER_REPLICATE), \
               cv2.warpAffine(mask, M, (dst_w, dst_h), flags=flags, borderMode=cv2.BORDER_REPLICATE)
...
class CVRandomPerspective(object):
    ...
    def __call__(self, img, mask):
        ...
        mask = cv2.warpPerspective(mask, M, (max_x, max_y), flags=flags, borderMode=cv2.BORDER_REPLICATE)
        mask = mask[min_y:, min_x:]
        return img, mask
...
class CVRescale(object):
    ...
    def __call__(self, img, mask):
        if self.factor == 0: return img, mask
        src_h, src_w = img.shape[:2]
...
class CVGeometry(object):
    ...
    def __call__(self, img, mask):
        if random.random() < self.p:
            ...
            aug_img, mask = self.transforms(img, mask)
            return aug_img, mask
        else:
            return img, mask
...
class CVDeterioration(object):
    def __init__(self, var, degrees, factor, p=0.5):
        ...
        transforms = Compose_(transforms)
