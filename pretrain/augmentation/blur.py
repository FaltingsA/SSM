...
from .ops import clipped_zoom, disk, plasma_fractal
'''
    PIL resize (W,H)
'''
class GaussianBlur:
...
class MotionBlur:
    ...
    def __call__(self, img, mag=-1, prob=1.):
        ...
        op = iaa.MotionBlur(k=16, angle=[-45, 45])

        img = Image.fromarray(op(image=np.asarray(img)))
...
class GlassBlur:
    ...
    def __call__(self, img, mag=-1, prob=1.):
        ...
        img = np.uint8(gaussian(np.array(img) / 255., sigma=c[0], channel_axis=True) * 255)
        ...
        img = np.clip(gaussian(img / 255., sigma=c[0], channel_axis=True), 0, 1) * 255
